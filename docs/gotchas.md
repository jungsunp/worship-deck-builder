...
Lyric transcription is a **free local hybrid** (no API key): Apple Vision OCR via `swift src/worship_deck/lyrics/ocr_ko.swift <img>` (needs Xcode Command Line Tools; groups observations by baseline into whole lines) → a local Ollama model reassembles syllables into lyric lines. Set up: `brew install ollama && ollama serve && ollama pull qwen3:14b`. Env: `OLLAMA_MODEL` (default `qwen3:14b`, used by **both** reassembly and phrase splitting), `OLLAMA_HOST` (default `http://127.0.0.1:11434`). 2026-06-11 single-model bake-off on 10 real sheets (harness + per-run details in `data/work/bench/`): `qwen3:14b` + the v3 prompt + `think: true` won — thinking measurably improves its recall/spacing, and the prompt's hard hyphen postcondition, worked OCR examples, faithfulness rule, and 절-번호 stripping each fixed a observed failure class. Keep melisma `-` in the model *input* (pre-stripping them in code degraded its joins). `qwen3.5:9b` paraphrases lyrics (산성→성벽) — disqualifying; `qwen3.5:27b` has the best output quality but ~100s/sheet and needs `sudo sysctl iogpu.wired_limit_mb=20480` (resets on reboot); `qwen3:30b-a3b` leaves orphaned particles; `gpt-oss:20b` returns *empty* on `/api/generate` whenever a schema `format` is attached (works only via `/api/chat` + `think:"low"`). Output falls back to the `thinking` field when `response` is empty. Feeding the image directly to the Ollama *vision* model crashed its runner on a real sheet — running reassembly as a **text** task on the Vision OCR output is why it's reliable.
...
`lyrics/linebreak.rebreak()` runs inside `transcribe()` on both paths: collapses adjacent repeated phrases to `… X N` (rule-based), then splits lines longer than `MAX_CHARS` (22 — measured lyric-banner geometry, see the module docstring) at a musical-phrase boundary via Ollama, falling back to a greedy space-split on any failure, so Ollama is never a hard dependency of the gasazip path. The split task uses the same `OLLAMA_MODEL` as reassembly (`qwen3:14b`). 2026-06-11 bake-off: `qwen3:14b` 5/6 valid phrase splits (the one reject is caught by per-line character-preservation validation → rule-based fallback); `qwen2.5:14b` and `exaone3.5:7.8b` only 2/5 — both re-segmented content *across* the batched input lines. `qwen3:14b`'s old renumbering failure on reassembly (which once forced a separate `OLLAMA_SPLIT_MODEL`) was a prompt problem, fixed by the v3 reassembly prompt's 절-번호 rule — the env var is gone.
