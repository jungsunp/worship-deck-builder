...
- **Open once, save once (#117).** Opening/saving the ~30 MB `master.key` is the dominant cost, so a build does it ~twice, not ~55×: `build()` runs `_ensure_keynote_ready()` (closes every doc, polls the count to 0), `save_draft.applescript` opens the template and `save … in outPath` (writes a pristine draft copy), and **every** mutating/reading primitive then targets the open `front document` — no per-call `open`/`save`/`close`. `finalize.applescript` does the single end-of-build save. **`save … in outPath` does NOT rebind the open document** — it stays bound to the *template* — so finalize MUST `save front document in (POSIX file outPath)` with the explicit path. A bare `save front document` writes the week's edits straight into `master.key` and corrupts the template (the on-disk draft, written pristine at `save_draft` time, then has none of the edits — the files end up swapped). Because the precondition forces zero open docs, `front document` is unambiguously the draft. Corollary for Mac-only tests/probes: read back via `front document` (the edits are in-memory until finalize), don't re-`open` the path — and don't `close` after reading or you'll discard unsaved edits.
