...
- `build()` locates every section by a **hard-coded slide index** keyed to the current `master.key` (찬양 6, 예배의 부름 48, 성가대 77, 봉헌 97, 교회소식 117, 말씀 verses 129, 말씀 title 134, and the unconditional `delete_slides(135, 18)` that drops the week's ad-hoc 말씀 special block, #97). `config/slide_map.yaml` documents the same numbers. Replacing `master.key` silently invalidates these — re-verify and update both files per the maintenance checklist in `README.md` ("Replacing the template"). No automatic drift handling yet (#98).
...
- **Keynote AppleScript gotchas** (osascript primitives, aspect-fill images, PNG-export verify, deck-text diffing) — read before any AppleScript work: see `docs/gotchas.md`.
- **Lyric transcription** is a free local hybrid (Apple Vision OCR + Ollama, `OLLAMA_MODEL`/`OLLAMA_HOST`, model bake-off) — setup & model notes: see `docs/gotchas.md`.
